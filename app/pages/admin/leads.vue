...
const filtered = computed(() => {
  ...
  if (statusFilter.value === 'all') return data.value.leads
...
function createEventFromLead(lead: Lead) {
  ...
  if (lead.phone) params.set('owner_phone', lead.phone.replace(/\D/g, ''))
...
<template>
  <div>
    <div class="mb-8 flex items-end justify-between gap-4">
      <h1 class="heading-display-md">Лиды</h1>
    ...
    <div class="mb-6 flex flex-wrap items-center gap-2">
      ...
      >
        <template v-if="s === 'all'">Все</template>
    ...
    >
      <h2 class="text-xl">Лидов в этом фильтре нет</h2>
      <p class="mt-2 text-(--color-muted-foreground)">Лиды появляются автоматически когда пара заполняет форму на лендинге.</p>
    </div>
